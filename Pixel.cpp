...
bool Pixel::compareThreshold(const Pixel &pixel, int threshold, bool isPremultiplied) const {
    int compare = (abs(getR() - pixel.getR()) + abs(getG() - pixel.getG()) + abs(getB() - pixel.getB()) + abs(getA() - pixel.getA())) / 4;
    //int compare = (abs(getR() - pixel.getR()) + abs(getG() - pixel.getG()) + abs(getB() - pixel.getB())) / 3;
    //qDebug("Compare: %d", compare);
